...
define(["jQuery","bootstrapJs"],function($){
    ...
    $(function () {
        ...
        placeholder.blur(function(){
            $(this).attr("placeholder","请输入您的工程编号");
            if($(this).css("color")!="rgb(245, 79, 79)"){
        ...
        function illegalOrder(){
            ...
            if($.trim(orderNum)==""){
                $.ajax({url:"",success:function(){
                    placeholder.val("输入工程编号不能为空！").css({"color":"#f54f4f","border":"1px solid #f54f4f","border-right":"none"}).blur();
                }});
                return false;
            }else if(!regExp.test(orderNum)){
                $.ajax({url:"",success:function(){
                    placeholder.val("请输入12位有效工程编号！").css({"color":"#f54f4f","border":"1px solid #f54f4f","border-right":"none"}).blur();
                }});
                return false;
            }else{
                <!--模拟点击提交效果-->
